// FILE: sequenceTemp_exam.cxx
// Written by: Michael Main (main@colorado.edu) - Oct 31, 1997
// Non-interactive test program for the sequenceTemp class using a linked sequenceTemp
//
// DESCRIPTION:
// Each function of this program tests part of the sequenceTemp class, returning
...
#include "sequenceTemp.h"  // Provides the template sequenceTemp class
using namespace std;
...
const char DESCRIPTION[MANY_TESTS+1][256] = {
    "tests for sequenceTemp Class with a linked sequenceTemp",
...
// **************************************************************************
// bool test_basic(const sequenceTemp<double>& test, size_t s, bool has_cursor)
...
// **************************************************************************
// bool test_items(sequenceTemp<double>& test, size_t s, size_t i, double items[])
//   The function determines if the test sequenceTemp has the correct items
...
//   At this point, one more advance takes the cursor off the sequenceTemp.
//   If any of this fails, the return value is false.
//   NOTE: The test sequenceTemp has been changed by advancing its cursor.
// **************************************************************************
bool test_items(sequenceTemp<double>& test, size_t s, size_t i, double items[])
{
    bool answer = true;
    
    cout << "The cursor should be at item [" << i << "]" << " of the sequenceTemp\n";
    cout << "(counting the first item as [0]). I will advance the cursor\n";
    cout << "to the end of the sequenceTemp, checking that each item is correct...";
    cout.flush( );
    ...
    {   // The test.is_item( ) function returns false too soon.
        cout << "\n    Cursor fell off the sequenceTemp too soon." << endl;
        answer = false;
    }
    else if (i != s)
    ...
    {   // The test.is_item( ) function returns true after moving off the sequenceTemp.
        cout << "\n    The cursor was moved off the sequenceTemp,";
...
// **************************************************************************
// bool correct
//   (sequenceTemp<double>& test, size_t s, size_t cursor_spot, double items[])
//   This function determines if the sequenceTemp (test) is "correct" according to
//   these requirements:
...
// NOTE: The function also moves the cursor off the sequenceTemp.
// **************************************************************************
bool correct
(sequenceTemp<double>& test, size_t size, size_t cursor_spot, double items[])
{
    bool has_cursor = (cursor_spot < size); 

    // Check the sequenceTemp's size and whether it has a cursor.
    ...
    // If there is a cursor, check the items from cursor to end of the sequenceTemp.
    if (has_cursor && !test_items(test, size, cursor_spot, items))
    {
        cout << "Test of the sequenceTemp's items failed." << endl << endl;
        return false;
    }

    // Restart the cursor at the front of the sequenceTemp and test items again.
    ...
    cout << "All tests passed for this sequenceTemp." << endl << endl;
    return true;
}
...
int test1( )
{
    sequenceTemp<double> empty;                // An empty sequenceTemp
    sequenceTemp<double> test;                 // A sequenceTemp to add items to
    double items1[4] = { 5, 10, 20, 30 };  // These 4 items are put in a sequenceTemp
    double items2[4] = { 10, 15, 20, 30 }; // These are put in another sequenceTemp

    // Test that the empty sequenceTemp is really empty
    cout << "Starting with an empty sequenceTemp." << endl;
    if (!correct(empty, 0, 0, items1)) return 0;

    // Test the attach function to add something to an empty sequenceTemp
    cout << "I am now using attach to put 10 into an empty sequenceTemp." << endl;
    test.attach(10);
    if (!correct(test, 1, 0, items2)) return 0;
    
    // Test the insert function to add something to an empty sequenceTemp
    cout << "I am now using insert to put 10 into an empty sequenceTemp." << endl;
    test = empty;
    test.insert(10);
    if (!correct(test, 1, 0, items2)) return 0;
    
    // Test the insert function to add an item at the front of a sequenceTemp
    cout << "I am now using attach to put 10,20,30 in an empty sequenceTemp.\n";
    ...
    // Test the insert function to add an item in the middle of a sequenceTemp
    cout << "I am now using attach to put 10,20,30 in an empty sequenceTemp.\n";
    ...
    // Test the attach function to add an item in the middle of a sequenceTemp
    cout << "I am now using attach to put 10,20,30 in an empty sequenceTemp.\n";
...
//   of the sequenceTemp. Also tests that attach/insert work correctly when there is
//   no cursor. Returns POINTS[2] if the tests are passed. Otherwise returns 0.
// **************************************************************************
int test2( )
{
    const size_t TESTSIZE = 30;
    sequenceTemp<double> test;
    size_t i;

    // Put three items in the sequenceTemp
    cout << "Using attach to put 20 and 30 in the sequenceTemp, and then calling\n";
    ...
    cout << "Inserting 10, which should go at the sequenceTemp's front." << endl;
    cout << "Then calling advance three times to run cursor off the sequenceTemp ...";
    cout.flush( );
    test.insert(10);
    test.advance( ); // advance to the 20
    test.advance( ); // advance to the 30
    test.advance( ); // advance right off the sequenceTemp
    if (test.is_item( ))
    {
        cout << " failed." << endl;
        return false;
    ...
    // Attach more items until the sequenceTemp becomes full.
    // Note that the first attach should attach to the end of the sequenceTemp.
    cout << "Calling attach to put the numbers 40, 50, 60 ...";
    cout << TESTSIZE*10 << " at the sequenceTemp's end." << endl;
    for (i = 4; i <= TESTSIZE; i++)
        test.attach(i*10);

    // Test that the sequenceTemp is correctly filled.
    cout << "Now I will test that the sequenceTemp has 10, 20, 30, ...";
    ...
    if (test.is_item( ))
    {
        cout << "    There are too many items on the sequenceTemp." << endl;
        return false;
    }
...
int test3( )
{
    ...
    // Within this function, I create several different sequenceTemps using the
    ...
    // Build a sequenceTemp with three items 10, 20, 30, and remove the middle,
    // and last and then first.
    cout << "Using attach to build a sequenceTemp with 10,30." << endl;
    test.attach(10);
    test.attach(30);
    cout << "Insert a 20 before the 30, so entire sequenceTemp is 10,20,30." << endl;
    test.insert(20);
    if (!correct(test, 3, 1, items3)) return 0;
    cout << "Remove the 20, so entire sequenceTemp is now 10,30." << endl;
    test.start( );
    test.advance( );
    test.remove_current( );
    if (!correct(test, 2, 1, items2)) return 0;
    cout << "Remove the 30, so entire sequenceTemp is now just 10 with no cursor.";
    cout << endl;
    test.start( );
    test.advance( );
    test.remove_current( );
    ...
    cout << "Using attach to build another sequenceTemp with 10,30." << endl;
    test.attach(10);
    test.attach(30);
    cout << "Insert a 20 before the 30, so entire sequenceTemp is 10,20,30." << endl;
    test.insert(20);
    ...
    if (!correct(test, 2, 1, items2)) return 0;
    cout << "Set the cursor to the start and remove the 10," << endl;
    cout << "so the sequenceTemp should now contain just 30." << endl;
    test.start( );
    test.remove_current( );
    if (!correct(test, 1, 0, items1)) return 0;
    cout << "Remove the 30 from the sequenceTemp, resulting in an empty sequenceTemp." << endl;
    test.start( );
    test.remove_current( );
    if (!correct(test, 0, 0, items1)) return 0;

    // Build a sequenceTemp with three items 10, 20, 30, and remove the first.
    cout << "Build a new sequenceTemp by inserting 30, 10, 20 (so the sequenceTemp\n";
    ...
    // Just for fun, fill up the sequenceTemp, and empty it!
    cout << "Just for fun, I'll empty the sequenceTemp then fill it up, then\n";
    cout << "empty it again. During this process, I'll try to determine\n";
    cout << "whether any of the sequenceTemp's member functions access the\n";
...
{
    const size_t TESTSIZE = 30;
    sequenceTemp<double> original; // A sequenceTemp that we'll copy.
    ...
    // Test copying of an empty sequenceTemp. After the copying, we change original.
    cout << "Copy constructor test: for an empty sequenceTemp." << endl;
    sequenceTemp<double> copy1(original);
    original.attach(1); // Changes the original sequenceTemp, but not the copy.
    if (!correct(copy1, 0, 0, items)) return 0;

    // Test copying of a sequenceTemp with current item at the tail.
    cout << "Copy constructor test: for a sequenceTemp with cursor at tail." << endl;
    for (i=2; i <= 2*TESTSIZE; i++)
        original.attach(i);
    sequenceTemp<double> copy2(original);
    ...
    // Test copying of a sequenceTemp with cursor at the front.
    cout << "Copy constructor test: for a sequenceTemp with cursor at front." << endl;
    original.insert(2);
    original.start( );
    // Cursor is now at the front.
    sequenceTemp<double> copy4(original);
    original.start( );
    original.advance( );
    original.remove_current( ); // Delete 2 from original, but not the copy.
    if (!correct
    ...
    // Test copying of a sequenceTemp with no current item.
    cout << "Copy constructor test: for a sequenceTemp with no current item." << endl;
    original.insert(2);
    while (original.is_item( ))
        original.advance( );
    // There is now no current item.
    sequenceTemp<double> copy5(original);
    original.start( );
    original.advance( );
    original.remove_current( ); // Delete 2 from original, but not the copy.
    if (!correct
...
int test5( )
{
    ...
    // Test copying of an empty sequenceTemp. After the copying, we change original.
    cout << "Assignment operator test: for an empty sequenceTemp." << endl;
    sequenceTemp<double> copy1;
    copy1 = original;
    original.attach(1); // Changes the original sequenceTemp, but not the copy.
    if (!correct(copy1, 0, 0, items)) return 0;
...
{
    const size_t TESTSIZE = 30;
    sequenceTemp<double> testa, testi;
    double items[2*TESTSIZE];
    size_t i;
...
int main( )
{
	...
	cout << "If you submit this sequenceTemp class now, you will have\n";
