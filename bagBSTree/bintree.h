...
// each node.
// 
// ALIAS for the binary_tree_node<Item> template class:
...
//     (which will be nullptr if there is no child).
//
...
//     node_ptr may be nullptr to indicate the empty tree).
...
//     Precondition: node_ptr is a pointer to a node in a binary tree (or
//     node_ptr may be nullptr to indicate the empty tree). If the pointer is
//     not nullptr, then depth is the depth of the node pointed to by node_ptr.
...
//     be nullptr for the empty tree).
//     Postcondition: All nodes at the root or below have been returned to the
//     heap, and root_ptr has been set to nullptr.
//
//   template <class Item>
...
template <class Item, class SizeType>
void pretty_print(binary_tree_node<Item>* node_ptr, SizeType depth);
